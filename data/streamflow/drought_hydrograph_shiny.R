...
#           light gray  -> axes and generic graphic elements (for dark theme)
#
# Fonts:    Inter (body), Crimson Text (titles), IBM Plex Mono (numbers)
#
# This version is modified for integration with the IRISCC Shiny app.
# Use the helper functions drought_hydrograph_shiny() or drought_hydrograph_from_xts()
# to generate plots from NetCDF files or pre-loaded xts objects.
#
# Usage in Shiny (via rpy2):
#   source("drought_hydrograph_shiny.R")
#   png_path <- drought_hydrograph_shiny(
#     qobs_path = "/path/to/discharge.nc",
#     gauge_id = "0006112080",
#     decade_years = 1960:1969,
#     persistence = 1,
#     station_name = "Station Name",
#     station_river = "River Name",
#     station_country = "DE",
#     station_area = "1234 km2"
#   )
# =============================================================================

# ---- packages ---------------------------------------------------------------
# Packages are assumed to be already installed. If not, install them manually:
...
# ---- global style -----------------------------------------------------------
# Fonts matching the Shiny app theme:
# - Inter: Clean sans-serif for body text, axis labels, legends
# - Crimson Text: Elegant serif for plot title
# - IBM Plex Mono: Professional monospace for numeric labels
FONT       <- "Inter"       # body font for most elements
FONT_TITLE <- "Crimson Text"  # title font
FONT_MONO  <- "IBM Plex Mono" # monospace for numbers

# Per-element font sizes
FS_AXTEXT  <- 14            # axis tick labels
FS_AXTITLE <- 16           # axis titles
FS_LEGEND  <- 14           # legend text
FS_TITLE   <- 20           # plot titles (match app's theme_config.py)

# Colors - kept as original for visual consistency
...
col_axis  <- "#eeeeee"  # light gray  - axes and generic elements (changed for dark theme)

FILL_ALPHA <- 0.60         # 60% opacity for all fills
LWD_MAIN   <- 1.4          # hydrograph line width
LWD_RP     <- 0.6          # RP line width

# Shared theme: Inter font for body, Crimson Text for title, per-element sizes, ticks INSIDE, no legend box,
  ...
  theme_classic(base_size = FS_AXTEXT, base_family = FONT) +
    theme(
      ...
      plot.title        = element_text(colour = col_axis, size = FS_TITLE,
                                       margin = margin(b = 24), family = FONT_TITLE),
      plot.title.position = "plot",
      legend.background = element_blank(),
      legend.key        = element_blank(),
      legend.text       = element_text(size = FS_LEGEND, family = FONT)
    )
}
...
#' Convert daily discharge data to monthly and create drought hydrograph
#' 
#' This function is designed for integration with Shiny apps via rpy2.
#' It reads daily discharge from a NetCDF file, aggregates to monthly,
#' and creates a drought hydrograph plot.
#'
#' @param qobs_path Path to NetCDF file containing observed discharge
#' @param qobs_varname Variable name for observed discharge (default: "Qobs")
#' @param gauge_id Gauge ID (zero-padded 10-digit string, e.g., "0006112080")
#' @param decade_years Integer vector of years to plot (default: 1960:1969)
#' @param persistence Minimum consecutive months below threshold to count as drought (default: 1)
#' @param station_name Station name for plot title
#' @param station_river River name for plot title
#' @param station_country Country for plot title
#' @param station_area Catchment area (character string, e.g., "1234 km2")
#' @param output_dir Directory to save the output PNG (default: tempdir())
...
#'   station_river = "Sample River",
#'   station_country = "DE",
#'   station_area = "1234 km2"
#' )
drought_hydrograph_shiny <- function(qobs_path, qobs_varname = "Qobs",
                                      gauge_id, decade_years = 1960:1969,
                                      persistence = 1,
                                      station_name, station_river, station_country,
                                      station_area, output_dir = tempdir(),
                                      filename = NULL) {
  ...
  nc <- nc_open(qobs_path)
  on.exit(nc_close(nc))
  
  # Extract time and discharge data for this gauge
  time_hours <- nc$var[["time"]]$vals
  base_dt <- as.POSIXct("1950-01-01", tz = "UTC")
  time_dates <- base_dt + time_hours * 3600  # Convert hours to POSIXct
  
  # Get observed discharge
  qobs_var <- nc$var[[qobs_varname]]
  qobs <- qobs_var$vals[, as.integer(gauge_id)]
  
  # Handle missing values
  qobs[is.na(qobs)] <- NA
  
  # Create monthly xts object
  monthly_q <- xts(qobs, order.by = as.Date(time_dates))
  monthly_q <- apply.monthly(monthly_q, mean, na.rm = TRUE)
  
  # Remove rows with all NA
  monthly_q <- monthly_q[!apply(is.na(monthly_q), 1, all)]
...
#' Create drought hydrograph from pre-loaded discharge data
#'
#' This variant accepts pre-loaded xts objects for both observed and simulated
#' discharge. It uses observed data for the drought analysis.
#'
#' @param qobs_xts xts object of monthly observed discharge
#' @param qsim_xts xts object of monthly simulated discharge (optional)
...
#' @export
drought_hydrograph_from_xts <- function(qobs_xts, gauge_id,
                                         station_name, station_river, station_country,
  ...
  # Validate xts object
  if (!inherits(qobs_xts, "xts")) {
    stop("qobs_xts must be an xts object")
  }
...
drought_hydrograph_from_dates <- function(dates, values, gauge_id,
  ...
  colnames(x) <- "Q"  # Name the column so the plotting function can find it
